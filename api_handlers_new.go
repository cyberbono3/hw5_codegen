...
func (h *MyApi) create(w http.ResponseWriter, r *http.Request) {
	...
	if paramname_name == "" {
		paramname_name = strings.ToLower(name)
	}
	...
	createParams := CreateParams{login, paramname_name, status, age}
...
func (h *OtherApi) create(w http.ResponseWriter, r *http.Request) {
	...
	if !(len(username) > 3) {
		js, _ := json.Marshal(JsonError{"login len must be >= 3"})
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		w.Write(js)
		return
	...
	newuser, err := h.Create(ctx, otherCreateParams)
	...
	js, _ := json.Marshal(ResponseOtherApiCreate{newuser, JsonError{""}})
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	w.Write(js)
}
